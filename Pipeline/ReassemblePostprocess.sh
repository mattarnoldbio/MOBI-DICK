...
echo -e '\t '         --sensitive '\t\t\t'  run assembly in sensitive mode
...
#   if [[ -d ${directory}/megahit_out ]]; then
#     megahit_out=$(ls ${directory}/megahit_out | grep contigs.fa)
#     contig_file_length=$(wc -l $directory/megahit_out/$megahit_out | awk '{print $1}') # Get number of lines in contig file
...
#   [[ -z $metadata ]] && [[ -z $genome ]] && echo No host genome or metadata file specified, skipping host filtering # If neither a metadata file or host genome was provided, skip host filtering
  ## UNZIP READS
  #pigz -d $directory/*unmap*.gz # Unzip reads

  ## ASSEMBLE READS
  #${install_path}Assembly.sh -r $directory -o $directory -t $threads --no_host_filtering $no_host_filtering -s $sensitive # Run assembly
