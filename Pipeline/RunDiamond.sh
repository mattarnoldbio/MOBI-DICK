...
diamond blastx -d $database -q ${data_dir}/all_contigs.fa -o ${data_dir}/all_contigs.${file_string}diamond.txt --outfmt 6 -p $threads 

for directory in $(ls -d $data_dir/*/); do
    accession=$(basename $directory)
    cat ${data_dir}/all_contigs.${file_string}diamond.txt | grep ${accession} > ${data_dir}/${accession}/${accession}.${file_string}diamond.txt
