...
echo -e '\t '          -c --min_contig_length '\t'  minimum contig length to keep "(default = 150)"
...
## RUN DIAMOND
${install_path}RunBLASTReads.sh -d $data_dir -t $threads -w diamond -k $krona_tools_db -x $xtra_fast # Run diamond on all contigs (for perfomrance reasons, this is done on a file of all contigs concatenated together)
